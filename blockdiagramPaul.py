...
def create_block_diagram(strat,dx,ve,xoffset,yoffset,scale,ci,strat_switch,contour_switch,bottom,topo_min,topo_max):
    ...
    gray = (0.6,0.6,0.6) # color for plotting sides used to (0.6,0.6,0.6)
...
def add_stratigraphy_to_block_diagram(strat,facies,h,thalweg_z,dx,ve,xoffset,yoffset,scale,layers_switch,color_mode,colors,line_thickness,export):
    ...
    for layer_n in range(ts-1): # main loop
        if np.mod(layer_n,3)==1:
        #if np.mod(layer_n,4)==2:
            color = (0.5,0.5,0.5) #  Used to be: color for sand - color = (1.0,1.0,0)
        else:
            color = (1.0,1.0,0) # color for mud PAUL EDIT FROM (0.6,0.3,0) was (0.5,0.5,0.5)
        ...
        if layers_switch == 1:
            ...
            mlab.plot3d(X1,Y1,Z1,color=(0,0,0),tube_radius=line_thickness) #paul edit color was= (0,0,0)
...
def create_exploded_view(strat,facies,topo,h,nx,ny,gap,dx,ve,scale,strat_switch,layers_switch,contour_switch,color_mode,colors,line_thickness,bottom,export,ci):
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            yoffset = (x1+i*gap)*dx
            #pdb.set_trace()
            create_block_diagram(strat[y1:y2,x1:x2,:],dx,ve,xoffset,yoffset,scale,ci,strat_switch,contour_switch,bottom,topo_min,topo_max)
